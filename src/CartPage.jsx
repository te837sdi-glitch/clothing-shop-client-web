...
function CartPage() {
    ...
    const handleCheckout = async () => {
        ...
        if (res) {
            ...
            alert('Помилка оформлення замовлення, можливо нестача товару певного розміру!');
        }
    };
    ...
    return (
        <div className="cart-page-container">
            <div className="cart-page-header">
                <button className="back-btn" onClick={() => navigate(-1)}>← Повернутися до магазину</button>
                <h2>Ваш кабінет покупок</h2>
            </div>
            <div className="cart-page-tabs">
                <button className={`tab-btn ${activeTab === 'cart' ? 'active' : ''}`} onClick={() => setActiveTab('cart')}>Кошик</button>
                <button className={`tab-btn ${activeTab === 'orders' ? 'active' : ''}`} onClick={() => setActiveTab('orders')}>Замовлення</button>
                <button className={`tab-btn ${activeTab === 'past_orders' ? 'active' : ''}`} onClick={() => setActiveTab('past_orders')}>Минулі замовлення</button>
            </div>
            <div className="cart-page-content">
                {activeTab === 'cart' && (
                    <div className="cart-section">
                                ...
                                {cartData.items.map((item, index) => (
                                    <div key={index} className="cart-item-row">
                                        ...
                                        <div className="cart-item-details">
                                            ...
                                            <p className="cart-item-meta">Розмір: <strong>{item.size}</strong> • {item.productId?.price} ₴/шт.</p>
                                            <div className="quantity-controls">
                                                <button disabled={item.quantity <= 1} onClick={async () => { let res = await api('updateCartQuantity', 'PUT', { productId: item.productId._id, size: item.size, action: 'minus' }); if (res) loadCartAndOrders(); }} className="qty-btn">-</button>
                                                <span className="qty-display">{item.quantity} шт.</span>
                                                <button onClick={async () => { let res = await api('updateCartQuantity', 'PUT', { productId: item.productId._id, size: item.size, action: 'plus' }); if (res) loadCartAndOrders(); }} className="qty-btn">+</button>
                                            </div>
                                        </div>
                                        <div className="cart-item-actions">
                                            <strong className="item-total">{item.quantity * item.productId?.price} ₴</strong>
                                            <button onClick={async () => { if (window.confirm(`Видалити товар "${item.productId?.title}" (Розмір: ${item.size}) з кошика?`)) { let res = await api('removeFromCart', 'POST', { productId: item.productId._id, size: item.size }); if (res) loadCartAndOrders(); } }} className="remove-btn">Видалити</button>
                                        </div>
                                    </div>
                                ))}
                                <div className="cart-total-section">
                ...
                {activeTab === 'orders' && (
                    <div className="orders-section">
                        {ordersData.filter(o => o.status === 'В обробці' || o.status === 'В дорозі').length === 0 ? (
                            ...
                            ordersData.filter(order => order.status === 'В обробці' || order.status === 'В дорозі').map(order => (
                                <div key={order._id} className="order-card active-order">
                                    <div className="order-header">
                                        <span><strong>Замовлення від:</strong> {new Date(order.date).toLocaleDateString()}</span>
                                        <span className={`order-status ${order.status === 'В дорозі' ? 'status-transit' : 'status-processing'}`}>{order.status === 'В дорозі' ? '🚚 Відправлено' : '⏳ В обробці'}</span>
                                    </div>
                                    <div className="order-items">
                                        {order.items.map((item, i) => (
                                            <div key={i} className="order-item-mini">
                ...
                {activeTab === 'past_orders' && (
                    <div className="orders-section">
                        {ordersData.filter(o => o.status === 'Доставлено').length === 0 ? (
                            ...
                            ordersData.filter(order => order.status === 'Доставлено').map(order => (
                                <div key={order._id} className="order-card completed-order">
                                    <div className="order-header">
                                        <span><strong>Завершене замовлення від:</strong> {new Date(order.date).toLocaleDateString()}</span>
                                        <span className="order-status status-delivered">✓ Доставлено</span>
