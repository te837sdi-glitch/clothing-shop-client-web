...
function ContentShop({ authorized, searchData }) {
  ...
  useEffect(() => {
    const fetchTypes = async () => {
        try {
            const response = await api('getTypes', 'GET');
            if (response) setDynamicTypes(response);
        } catch (error) {
            console.error('Помилка завантаження типів:', error);
        }
    };
  ...
  async function fetchShopProducts() {
    try {
        let requestData = {
            page: currentPage, limit: limit, search: searchData,
            category: selectedCategory, types: selectedTypes,
  ...
  useEffect(() => { fetchShopProducts(); }, [currentPage, searchData, filtersApplied]);

  const applyFilters = () => { setCurrentPage(1); setFiltersApplied(filtersApplied + 1); };
  ...
  const handleCategoryChange = (category) => {
    ...
    const conflictingCategory = category === 'forWoman' ? 'forMan' : 'forWoman';
    const forbiddenTypeValues = dynamicTypes.filter(t => t.category === conflictingCategory).map(t => t.value);
    setSelectedTypes(prevSelected => prevSelected.filter(chosenType => !forbiddenTypeValues.includes(chosenType)));
  };
    ...
    <div className="contentShop">
      <aside className="filters">
        ...
        <div className="filter-group">
          <h4>Ціна (₴)</h4>
          <div className="price-inputs">
            <input type="number" placeholder="Від 0" onChange={(e) => setMinPrice(e.target.value !== '' ? Number(e.target.value) : 0)} />
            <span> - </span>
            <input type="number" placeholder="До 10000" onChange={(e) => setMaxPrice(e.target.value !== '' ? Number(e.target.value) : 10000)} />
          </div>
        </div>
        ...
        <div className="filter-group">
          <h4>Типи</h4>
          <ul className="filter-list">
            <li><label><input type="radio" name="category" checked={selectedCategory === 'allProducts'} onChange={() => handleCategoryChange('allProducts')} />Всі товари</label></li>
            <li><label><input type="radio" name="category" checked={selectedCategory === 'forMan'} onChange={() => handleCategoryChange('forMan')} />Чоловічий одяг</label></li>
            <li><label><input type="radio" name="category" checked={selectedCategory === 'forWoman'} onChange={() => handleCategoryChange('forWoman')} />Жіночий одяг</label></li>
          </ul>
        </div>
        ...
        <div className='filter-group'>
          ...
          <ul className='filter-list'>
            {dynamicTypes
              .filter(typeObj => selectedCategory === 'allProducts' || typeObj.category === 'unisex' || typeObj.category === selectedCategory)
              .map(typeObj => (
              <li key={typeObj.value}>
                <label>
                  <input type='checkbox' checked={selectedTypes.includes(typeObj.value)} onChange={() => handleTypeChange(typeObj.value)} /> 
                  {typeObj.value}
                </label>
              </li>
            ))}
        ...
        <button className="apply-filters-btn" onClick={applyFilters}>Застосувати фільтри</button>
      </aside>

      <dialog className='aboutProduct' ref={modalChoiseProductRef}>
        <div className="modal-container">
          <button className="close-modal-btn" onClick={(e) => { e.preventDefault(); e.stopPropagation(); modalChoiseProductRef.current.close(); }}>&#10005;</button>
          ...
          <div className="modal-content">
            ...
            <div className="modal-details">
              ...
              <div className="modal-stock">
                <span>В наявності:</span> 
                <strong>{choiseProduct.sizes?.reduce((sum, s) => sum + s.count, 0) > 0 ? `${choiseProduct.sizes.reduce((sum, s) => sum + s.count, 0)} шт.` : "Немає в наявності"}</strong>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* ОНОВЛЕНЕ МОДАЛЬНЕ ВІКНО ВИБОРУ РОЗМІРУ */}
      <dialog className="modal-dialog" ref={modalSizeRef} style={{ width: '350px' }}>
        <div className="modal-header">
            <p className="modal-title">Оберіть розмір</p>
            <button className="close-btn" onClick={(e) => { e.preventDefault(); modalSizeRef.current.close(); }}>✕</button>
        </div>

        <div className="modal-content" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
            {productToCart?.sizes && productToCart.sizes.length > 0 ? (
                <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    {productToCart.sizes.map((s, idx) => (
                        <button
                            key={idx}
                            disabled={s.count === 0}
                            ...
                            style={{
                                padding: '10px 15px',
                                border: selectedSize === s.size ? '2px solid #007bff' : '1px solid #ccc',
                                background: s.count === 0 ? '#f5f5f5' : 'white',
                                cursor: s.count === 0 ? 'not-allowed' : 'pointer',
                                borderRadius: '5px',
                                color: s.count === 0 ? '#aaa' : '#333'
                            }}
                        >
                            {s.size} {s.count === 0 && '(Немає)'}
                        </button>
                    ))}
                </div>
            ) : (
            ...
            <button 
                className="submit-btn" 
                style={{ width: '100%', opacity: !selectedSize ? 0.5 : 1, marginTop: '10px' }}
                ...
                onClick={async (e) => {
                    ...
                    if (response) {
                        alert(`Товар (Розмір: ${selectedSize}) успішно додано у кошик!`);
                        modalSizeRef.current.close();
                    } else {
                        alert('Помилка при додаванні у кошик');
                    }
      ...
      <div className='productContainer'>
          <main className="products">
            {products === null ? (
              ...
              products.map((product) => (
                <div onClick={(e) => { e.stopPropagation(); openChoiseModal(product); }} key={product._id} className="product-card">
                  ...
                  <div className="product-info">
                    ...
                    <button onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      if (authorized === false) return alert('Будь ласка, увійдіть або створіть акаунт!');
                      setProductToCart(product); 
                      setSelectedSize('');
                      modalSizeRef.current.showModal(); 
                  }} className="add-to-cart-btn">У кошик</button>
          ...
          {totalPages > 1 && (
            <div className="pagination-controls">
              <button onClick={handlePrevPage} disabled={currentPage === 1} className="pag-btn" style={{ cursor: currentPage === 1 ? 'not-allowed' : 'pointer' }}>← Попередня</button>
              <span className="pagination-info">Сторінка <strong>{currentPage}</strong> з <strong>{totalPages}</strong></span>
              <button onClick={handleNextPage} disabled={currentPage === totalPages} className="pag-btn" style={{ cursor: currentPage === totalPages ? 'not-allowed' : 'pointer' }}>Наступна →</button>
            </div>
          )}
      </div>
    </div>
